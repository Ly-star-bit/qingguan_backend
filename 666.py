        ...
        for item in api_data["items"]:
            try:
                ...
                product = {
                    ...
                    # **修改点**: 假设API现在提供重量范围
                    "min_weight_per_box": float(item.get("min_weight", 15.0)), # 示例：增加最小重量字段
                    "max_weight_per_box": float(item.get("max_weight", 17.0)), # 示例：增加最大重量字段
                }
...
def optimize_packing_selection(
    ...
    # **新增变量**: w[i]: 产品i的总重量 (连续)
    w: dict = lp.LpVariable.dicts("total_weight", range(n), lowBound=0, cat="Continuous")


    # --- 目标函数：最小化总货值（USD） ---
    model += lp.lpSum([value_per_box_usd[i] * b[i] for i in range(n)]), "Total_Value_USD"
    ...
    if lp.LpStatus[model.status] == lp.LpStatus[1]:  # Optimal
        ...
        for i in range(n):
            ...
            if boxes_count > 0:
                ...
                product_result = {
                    ...
                    "avg_weight_per_box": avg_weight_per_box,
        ...
        if expansion_factor:
            print(f"有膨胀系数，不需要验证单位税金, 税金/重量 = {tax_per_weight_cny:.2f} CNY/kg ✅")
        else:
            print(
...
if __name__ == "__main__":
    ...
    upload_data = {
        "products_data": [
            {
                ...
                "pcs_per_box": 17,
                "tax_rate": 0.311,
                "min_weight_per_box": 15.5, # 新增
                "max_weight_per_box": 17.0, # 新增
            },
            {
                "name": "女士体恤/上衣",
                "price": 0.75,
                "pcs_per_box": 18,
                "tax_rate": 0.326,
                "min_weight_per_box": 16.0, # 新增
                "max_weight_per_box": 17.5, # 新增
            },
            {
                "name": "手机/电脑支架",
                "price": 0.4,
                "pcs_per_box": 16,
                "tax_rate": 0.321,
                "min_weight_per_box": 15.0, # 新增
                "max_weight_per_box": 16.8, # 新增
            },
        ],
        "W_target": 4104,
        "B_target": 247,
        ...
        "exchange_rate": 7.22,
        "k": 3, # 改为2个品类，更容易找到解
        "min_boxes_per_product": 50, # 适当提高最小箱数
    }
...
#     Args:
#         products_data: 产品数据列表，必须包含 name, price, pcs_per_box, tax_rate, single_weight 字段
#         W_target: 目标总重量 (kg)
#         B_target: 目标总箱数
#         alpha: 货值/重量 最低比率 (美元/千克)
#         beta_cny: 税金/重量 最高比率 (人民币/千克)
#         exchange_rate: USD to CNY 汇率
#         k: 最多选择 k 个不同的产品品类
#         min_boxes_per_product: 每个产品最少箱数
...
#     # 数据预处理
#     # ======================
#     default_weight = W_target / B_target  # 缺失时默认单箱重量
#     n: int = len(products)

#     names: list = [p["name"] for p in products]
#     unit_price_usd = [p["price"] for p in products]  # 单件价格 (USD)
#     pcs_per_box = [p["pcs_per_box"] for p in products]
#     tax_rates = [p["tax_rate"] for p in products]  # 税率（比例）
...
#     # 单箱重量：使用 default_weight 补全 None
#     weights = [
#         p["single_weight"] if p["single_weight"]  else default_weight
#         for p in products
#     ]
...
#     # 约束条件
#     model += lp.lpSum([b[i] for i in range(n)]) == B_target, "Total_Boxes"
#     model += lp.lpSum([weights[i] * b[i] for i in range(n)]) == W_target, "Total_Weight"
...
#     for i in range(n):
#         model += b[i] <= B_target * z[i], f"Linking_Constraint_{i}"
#         model += b[i] >= min_boxes_per_product * z[i]
...
#                 selected_count += 1
#                 w = weights[i]
#                 val_usd = value_per_box_usd[i] * count
#                 tax_cny = tax_per_box_cny[i] * count
#                 pcs = pcs_per_box[i] * count
...
#                 print(
#                     f"{names[i]:<15} : {count:4.0f} 箱 × {w:5.2f}kg = {w*count:6.2f}kg | "
#                     f"货值: {val_usd:7.2f} USD | 税金: {tax_cny:6.2f} CNY | 件数: {pcs:6.0f}"
#                 )

#         print("-" * 90)
#         print(
#             f"总计        : {total_weight:6.2f}kg | 货值: {total_value_usd:8.2f} USD | "
#             f"税金: {total_tax_cny:7.2f} CNY | 选了 {selected_count}/{k} 品类"
#         )

#         # 验证比率
#         value_per_weight_usd = total_value_usd / total_weight
#         tax_per_weight_cny = total_tax_cny / total_weight
...
# def main_optimize_packing_selection():
#     """主函数 - 示例如何使用优化函数"""
#     # 默认产品数据示例
#     default_products = {
#         "products_data": [
...
#         "exchange_rate": 7.22,
#         "k": 3,
#         "min_boxes_per_product": 1,
#     }

#     # 尝试从API获取数据，如果失败则使用默认数据
#     api_token = get_qingguan_access_token()
#     print("正在获取产品数据...")
#     api_products = fetch_products_from_api(api_token=api_token)
#     if api_products:
#         products_data = api_products
#         print("使用API获取的产品数据")
#     else:
#         products_data = default_products
#         print("⚠️ API获取失败，使用默认产品数据")

#     # 调用优化函数
#     result = optimize_packing_selection(products_data)
#     return result
