...
def get_skudetails(
    ...
    try:
        ...
        if trackingnumber:
            trackingnumber_list = [trackingnumber.strip() for trackingnumber in trackingnumber.split(',')]
...
def update_skudetail(id: int, skudetail: SkuDetail, session: Session = Depends(get_session)):
    try:
...
async def upload_excel(file: UploadFile = Form(...), session: Session = Depends(get_session)):
    try:
        # 确保文件是Excel
        ...
        if missing_columns:
            raise HTTPException(status_code=400, detail=f"Excel缺少必要的列: {', '.join(missing_columns)}")
            
        # 计算每个组合的出现次数
        df['group'] = df.apply(lambda x: f"{x['原SKU']}_{x['新SKU']}_{x['PC编号']}", axis=1)
        ...
        for _, row in df.iterrows():
            ...
            # 检查数据库中是否已存在相同的记录
            existing_record = session.query(SkuDetail).filter(
            ...
            if existing_record:
                ...
                existing_record.remaining_download_count += current_count  # 同时更新剩余下载次数
                existing_record.trackingnumber = row['跟踪号'] if row['跟踪号'] else existing_record.trackingnumber  # 保留原值如果为空
                existing_record.boxno = row['箱号'] if row['箱号'] else existing_record.boxno  # 保留原值如果为空
...
async def upload_labels(
    ...
    try:
        ...
        for pdf_file in temp_dir.glob("**/*.pdf"):
            ...
            # 更新数据库
            skudetail = session.exec(select(SkuDetail).where(SkuDetail.newsku == sku)).first()
            if skudetail:
        ...
        if temp_dir.exists():
            shutil.rmtree(temp_dir)
        raise HTTPException(status_code=500, detail=str(e))
@skudetail_router.get("/skudetail/download_label_preview/{sku}", summary="下载SKU标签预览")
async def download_label_preview(sku: str, session: Session = Depends(get_session)):
    try:
        # 查询数据库获取标签URL
        skudetail = session.exec(select(SkuDetail).where(and_(
        ...
        return FileResponse(file_path, filename=f"{newsku}.pdf", media_type="application/pdf")
